...
namespace demo4 {
	/**
	 * @brief difference between const std::vector<T>::iterator
	 * and std::vector<T>::const_iterator
	 *
	 */
	...
	void main() {
		...
		const auto iter1 = vec.begin(); // const std::vector<int>::iterator
		*iter1 = 10; // can modify the value that the iterator points to
		// ++iter1; // cannot modify the iterator itself
		// iter1 = vec.end();

		auto iter2 = vec.cbegin(); // std::vector<int>::const_iterator
		// *iter2 = 10; // cannot modify the value that the iterator points to
		++iter2; // but can modify the iterator itself
		iter2 = vec.begin();
	}
...
namespace demo5 {
	void main() {
		 ...
		 *
		 */
		auto list = std::list<std::string>{"sator", "arepo", "tenet", "opera", "rotas"};
...
namespace demo6 {
	...
	void main() {
		...
		vector_it += 2;
		std::cout << "Direct access with random access iterator (std::vector): " << *vector_it
		          << std::endl; // 3
		...
		std::cout << "Indirect access with bidirectional iterator (std::list): " << *list_it
		          << std::endl;
	}
} // namespace demo6
...
#include <vector>
		...
		it = vec.begin() + 3; // Adjust the iterator to point to the same element as before
